...
# load dataset
df = read_csv('df_private_mean_time.csv', header=0, index_col=0, squeeze=True)


Towns = list(numpy.sort(list(set(df.town.values))))

#print([town for town in Towns for i in range(3)])

out = pd.DataFrame({'monthtx': [24213,24214,24215]*len(Towns),
					'town': [town for town in Towns for i in range(3)]})

predictions = list()


for _,town in enumerate(Towns):
	print('{}: {}/{}'.format(town,_+1,len(Towns)))

	series = df.loc[df.town == town].drop(columns = ['town']).set_index('monthtx').squeeze()
...
out.to_csv('pred_mean_private.csv')
