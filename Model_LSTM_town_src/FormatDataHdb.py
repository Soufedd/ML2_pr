...
monthly_mean = df.groupby(['monthtx', 'town'])['resale_price'].mean()
monthly_mean = pd.DataFrame(monthly_mean).reset_index()
monthly_mean.columns = ['monthtx', 'town', 'mean_resale_price']

monthly_mean.to_csv("df_hdb_mean_time.csv")

df = df.merge(monthly_mean, left_on = ['monthtx', 'town'], right_on = ['monthtx', 'town'])
