...
pred_mean = pd.read_csv('pred_mean_private.csv').drop(columns = 'Unnamed: 0')


df_test = df_test.merge(pred_mean, left_on = ['monthtx', 'town'], right_on = ['monthtx','town'])
...
lasso = make_pipeline(PartScaler(last_col = 6), Lasso(alpha =0.0001, random_state=1))
ENet = make_pipeline(PartScaler(last_col = 6), ElasticNet(alpha=0.001, l1_ratio=.5, random_state=3))
...
model_lgb = lgb.LGBMRegressor(num_leaves=200, n_estimators = 200, learning_rate = 0.15, random_state = 2)
...
dnn = make_pipeline(PartScaler(last_col = 6), KerasRegressor(build_fn=NeuralNetModel, epochs=10, batch_size=200, verbose=0))

stacked_averaged_models = StackingAveragedModels(base_models = [model_lgb],
                                                 meta_model = lasso,
                                                 n_folds = 2)
...
for Town in Towns:
  ...
  drop_cols_test = ['index', 'town']
  drop_cols_train = ['town', 'resale_price', 'mean_resale_price']#'resale_price']
...
if not (val and tcv and cv):
  write_file(Sub, 'private', date = False)
